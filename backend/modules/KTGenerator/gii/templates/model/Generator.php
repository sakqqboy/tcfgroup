...
//namespace common\gii\templates\model;
...
class Generator extends \yii\gii\Generator
{
 ...
 public function generateRules($table)
 {
  ...
  foreach ($table->foreignKeys as $refs) {
   ...
   $rules[] = "[['$attributes'], 'exist', 'skipOnError' => true, 'targetClass' => $refClassName" . "Master::className(), 'targetAttribute' => [$targetAttributes]]";
  }
   ...
   foreach ($db->getSchema()->getTableSchemas($schemaName) as $table) {
    ...
    foreach ($table->foreignKeys as $refs) {
     ...
     $relations[$table->fullName][$relationName] = [
      "return \$this->hasOne($refClassName" . "Master::className(), $link);",
      $refClassName,
      false,
     ];
